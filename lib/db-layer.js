...
async function createWaitlistEntry({ id, name, email }) {
  await pool.query(
    `INSERT INTO waitlist (id, name, email, status, created_at) VALUES ($1,$2,$3,'pending',$4)`,
...
async function getWaitlist() {
  ...
  return r.rows.map(w => ({ id: w.id, name: w.name, email: w.email, status: w.status, createdAt: w.created_at, approvedAt: w.approved_at }));
}

async function getWaitlistEntry(id) {
  const r = await pool.query('SELECT * FROM waitlist WHERE id = $1', [id]);
  if (!r.rows.length) return null;
  const w = r.rows[0];
  return { id: w.id, name: w.name, email: w.email, status: w.status, createdAt: w.created_at };
}

async function updateWaitlistStatus(id, status) {
  await pool.query('UPDATE waitlist SET status=$1, approved_at=$2 WHERE id=$3',
...
async function updateTenantProvisioned(id, { serviceUrl, renderServiceId }) {
  await pool.query(
    'UPDATE tenants SET service_url=$1, render_service_id=$2, status=$3 WHERE id=$4',
    [serviceUrl, renderServiceId, 'active', id]
  );
}
...
async function logSchemaChange({ schemaId, schemaName, action, fields }) {
  const id = Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
  await pool.query(
    `INSERT INTO schema_change_log (id, "schemaId", "schemaName", action, fields, "createdAt") VALUES ($1,$2,$3,$4,$5,$6)`,
    [id, schemaId, schemaName, action, JSON.stringify(fields || []), new Date().toISOString()]
  );
}

async function getSchemaChanges() {
  const res = await pool.query('SELECT * FROM schema_change_log ORDER BY "createdAt" DESC LIMIT 100');
  return res.rows.map(r => ({ ...r, fields: JSON.parse(r.fields) }));
}

async function markSchemaChangeReviewed(id) {
  await pool.query('UPDATE schema_change_log SET reviewed=1 WHERE id=$1', [id]);
}
